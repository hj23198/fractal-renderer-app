...
class Render {
  static renderFrame(json) {
    ...
    console.log(json);
    fs.writeFileSync(
      path.join(__dirname, "resources", "request.json"),
      JSON.stringify(json)
    );
  ...
  static saveImage() {
    ...
    fs.copyFileSync(
      path.join(__dirname, "resources", "fractal_image.png"),
      fpath + ".png"
    );
  }
  ...
  static renderVideo(e, id1, id2, frames, mod_amount) {
    ...
    for (let i = 0; i < frames - 1; i++) {
      delta_zoom_frame.push(
        Math.abs(zoom_values_frame[i + 1] - zoom_values_frame[i])
      );
    }
    ...
    for (let i = 0; i < frames; i++) {
      ...
      fs.renameSync(
        path.join(__dirname, "resources", "fractal_image.png"),
        path.join(__dirname, "video_out", "fractal_image_" + i + ".png")
      );
    }
  }
}
...
class Bookmark {
  static load() {
    bookmarks = fs.readFileSync(
      path.join(__dirname, "/resources/bookmarks.json")
    );
