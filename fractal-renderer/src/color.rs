...
pub fn color_depth_values(depth_values: Vec<Vec<u32>>, request: Value) -> RgbImage {
    ...
    if request_type == "log_grad" {
        ...
        let point2 = params["point2"]
            ...
            .map(|x| x.as_i64().unwrap() as i16)
            .collect::<Vec<i16>>();
        let set_color_vec = params["set_color"]
            .as_array()
            .unwrap()
            .iter()
        ...
        let colored_image = apply_log_grad(
            depth_values,
            xsize,
            ysize,
            point1,
...
fn apply_log_grad(
    ...
    let repetition_difference =
        ((minmax[1] as f64).log(base) - (minmax[0] as f64).log(base)) / repetitions as f64;
        ...
        for y_iter in 0..ysize {
            ...
            if value == 0 {
                ...
                image.put_pixel(
                    ...
                    Rgb([
                        (point_1[0]
                            + ((value as f64).log(base) * color_delta[0]) as i16 % mod_value as i16)
                            as u8,
                        (point_1[1]
                            + ((value as f64).log(base) * color_delta[1]) as i16 % mod_value as i16)
                            as u8,
                        (point_1[2]
                            + ((value as f64).log(base) * color_delta[2]) as i16 % mod_value as i16)
                            as u8,
                    ]),
                );
            }
