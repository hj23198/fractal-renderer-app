...
pub fn render(
    ...
    let searched: Arc<Mutex<Vec<Vec<bool>>>> =
        Arc::new(Mutex::new(vec![
            vec![false; ysize as usize];
            xsize as usize
        ]));
    let repetition_values: Arc<Mutex<Vec<Vec<u32>>>> =
        Arc::new(Mutex::new(vec![vec![0; ysize as usize]; xsize as usize]));
        ...
        threads.push(thread::spawn(move || loop {
            ...
            for xmod in next_to_value.iter() {
                for ymod in next_to_value.iter() {
                    if 0 <= image_x + xmod
                        && image_x + xmod < xsize as i64
                        && 0 <= image_y + ymod
                        && image_y + ymod < ysize as i64
                    {
                        if searched_areas[(image_x + *xmod) as usize][(image_y + ymod) as usize]
                            == false
                        {
